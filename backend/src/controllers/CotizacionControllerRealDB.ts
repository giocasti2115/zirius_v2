...
export const getAllCotizaciones = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Construir la consulta de manera más robusta sin parámetros para LIMIT/OFFSET
    let mainQuery = `
      SELECT 
        c.id,
        c.id_cliente,
    ...
    mainQuery += ` ORDER BY c.creacion DESC LIMIT ${limitNumber} OFFSET ${offset}`

    // Preparar parámetros finales - solo queryParams, no limit/offset
