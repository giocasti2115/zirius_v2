...
interface AprobarVisitaModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  visita: {
...
export default function AprobarVisitaModal({ 
  isOpen, 
  onClose, 
  onSuccess, 
  visita 
}: AprobarVisitaModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:3002/api/v1/real/visitas/${visita.id}/aprobar`, {
        method: 'PUT',
        headers: {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Error al aprobar la visita')
      }
    } catch (error: any) {
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-lg">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            ...
            <h2 className="text-xl font-bold text-gray-900">Aprobar Visita</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-4 bg-green-50 border border-green-200 rounded-md">
            <div className="flex items-center">
              ...
              <span className="text-green-800 font-medium">
                ¿Estás seguro de que deseas aprobar esta visita?
              </span>
            </div>
            <p className="text-green-700 text-sm mt-1">
              La visita pasará a estado "Abierta" y podrá ser ejecutada.
            </p>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            >
              ...
              {loading ? 'Aprobando...' : 'Aprobar Visita'}
            </button>
          </div>
        </form>
      </div>
