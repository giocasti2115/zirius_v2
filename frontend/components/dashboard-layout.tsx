...
const navigation = [
  ...
  },
  {
    ...
    icon: Package,
    badgeRoute: "warehouse",
    submodules: [
      { name: "Todas las Solicitudes", route: "warehouse-all" },
      { name: "Pendientes", route: "warehouse-pending" },
      { name: "Aprobadas", route: "warehouse-approved" },
      { name: "Despachadas", route: "warehouse-dispatched" },
  ...
  },
  {
    name: "Dar de Baja",
    icon: PhoneOff,
    badgeRoute: "decommission",
    submodules: [
      { name: "Solicitudes Dado de Baja", route: "decommission-all" },
      { name: "Solicitudes Baja Pendientes", route: "decommission-pending" },
  ...
  },
  {
    ...
    submodules: [
      ...
      { name: "Pendientes", route: "quotes-pending" },
  ...
  },
  {
    ...
    submodules: [
      ...
      { name: "Órdenes Abiertas Preventivo", route: "orders-open-preventive" },
      { name: "Órdenes Abiertas CIG", route: "orders-open-cig" },
  ...
  },
  {
    ...
    submodules: [
      ...
      { name: "Visitas Abiertas", route: "visits-open" },
...
export function DashboardLayout({ children, activeRoute = "dashboard", onNavigate }: DashboardLayoutProps) {
  ...
  const { getCountForRoute, getTotalForType } = useDynamicCounts()
  ...
  const toggleSection = (sectionName: string) => {
    setExpandedSections((prev) => {
      ...
      // Si no está expandida, cerramos todas las demás y abrimos esta
      return [sectionName]
    })
  }
  ...
  const handleNavigate = (route: string) => {
    if (onNavigate) {
      ...
      const routeMap: Record<string, string> = {
        ...
        'service-all': '/solicitudes',
        'quotes-all': '/cotizaciones',
        'config-equipment': '/equipos',
        'orders-all': '/ordenes',
        'visits-all': '/visitas',
        'config-users': '/usuarios',
        'config-locations': '/sedes',
        'connection-test': '/dashboard/test-api'
      }
  ...
  const renderNavItem = (item: (typeof navigation)[0], isMobile = false) => {
    ...
    const badgeCount = item.badgeRoute ? getTotalForType(item.badgeRoute) : (item.route ? getCountForRoute(item.route) : 0)
    ...
    if (!hasSubmodules) {
      return (
        ...
        >
          ...
          {(!sidebarCollapsed || isMobile) && badgeCount > 0 && (
        ...
        {(!sidebarCollapsed || isMobile) && isExpanded && (
          <div className="ml-8 mt-1 space-y-1">
            {item.submodules?.map((submodule) => {
              ...
              const subBadgeCount = getCountForRoute(submodule.route)
              return (
                <button
                  key={submodule.name}
