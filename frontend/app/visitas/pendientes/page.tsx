...
export default function VisitasPendientesPage() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (visitasData.success) {
        ...
        console.log('✅ Visitas pendientes cargadas:', visitasData.data.visitas.length);
      } else {
        setError('Error cargando visitas pendientes');
      }

    } catch (err) {
      console.error('❌ Error cargando visitas pendientes:', err);
  ...
  const calcularDiasVencimiento = (fecha: string) => {
    ...
    const diferencia = Math.ceil((fechaProgramada.getTime() - hoy.getTime()) / (1000 * 3600 * 24));
    return diferencia;
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Visitas Pendientes</h1>
          </div>
          <p className="text-sm text-gray-600">
            Visitas que requieren programación o reasignación
          </p>
        </div>
        ...
        <Card className="border-l-4 border-l-yellow-500">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium text-gray-600">Total Pendientes</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-red-600">
                  {visitas.filter(v => calcularDiasVencimiento(v.fecha_programada) < 0).length}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-orange-600">
                  {visitas.filter(v => {
                    const dias = calcularDiasVencimiento(v.fecha_programada);
        ...
        <Card className="border-l-4 border-l-gray-500">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium text-gray-600">Sin Técnico</p>
                <p className="text-xl font-bold text-gray-600">
                  {visitas.filter(v => !v.tecnico_nombre).length}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg">
              Visitas Pendientes ({totalRegistros.toLocaleString()} total)
            </CardTitle>
        ...
        <CardContent className="pt-0">
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mt-2">Cargando visitas pendientes...</p>
            </div>
          ) : (
                ...
                <div className="max-h-[500px] overflow-y-auto">
                  <Table>
                    ...
                    <TableBody>
                      {visitas.length > 0 ? (
                        visitas.map((visita) => {
                          const dias = calcularDiasVencimiento(visita.fecha_programada);
                          return (
                        ...
                        <TableRow>
                          <TableCell colSpan={9} className="text-center py-12 text-gray-500">
                            No hay visitas pendientes con los filtros aplicados
                          </TableCell>
                        </TableRow>
                      )}
                    </TableBody>
